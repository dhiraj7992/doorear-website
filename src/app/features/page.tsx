      ...
      <MarketingPageHero
        ...
        description={
          <>
            ...
            in one multi-tenant workspace. We stay transparent about what is live
            for your tenant—especially carrier integrations—so evaluation calls
            stay grounded in reality.
          </>
        }
        image={{
          src: '/images/marketing/page-features.jpg',
          alt: 'Cargo aircraft and logistics — freight and network operations',
        }}
        imageCaption='End-to-end visibility: from booking intent through hubs to proof of delivery—without losing the thread in spreadsheets.'
      ...
      <section className='border-b border-[var(--app-border)] bg-[var(--app-surface)] py-16 md:py-20'>
        <MotionInView className='marketing-container space-y-12'>
          <SectionHeading
            ...
            description='Each module maps to how dispatchers, hub managers, and leadership actually run the day—from AWB booking system India workflows to fleet and hub management logistics visibility.'
          />
          <FeatureGrid items={capabilities} columns={3} />
        </MotionInView>
      </section>

      <section className='relative overflow-hidden border-y border-[var(--app-border)] bg-[var(--app-card)] py-16 md:py-20'>
        <div
          className='pointer-events-none absolute right-0 top-0 h-64 w-64 rounded-full bg-[var(--app-primary)]/5 blur-3xl'
          aria-hidden
        />
        <MotionInView className='marketing-container relative'>
          <div className='grid gap-10 lg:grid-cols-2 lg:items-start'>
            <div>
              ...
              <p className='mt-4 text-base leading-relaxed text-[var(--app-muted)]'>
                Plans include monthly shipment ceilings and user seats. When
                volumes spike—festival season, new client onboarding, or e‑commerce
                surge—you can add{' '}
                <strong className='font-semibold text-[var(--app-foreground)]'>
                  extra shipments at a predictable per-piece top-up
                </strong>{' '}
                instead of forcing an immediate tier jump. That flexibility is a
                meaningful differentiator for operators balancing growth and
                working capital.
              </p>
              <ul className='mt-6 space-y-3 text-sm text-[var(--app-muted)]'>
                <li className='flex gap-2'>
                  ...
                  Aligns finance and ops on the same usage story inside the
                  product—not a disputed spreadsheet.
                </li>
                <li className='flex gap-2'>
                  <span className='mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--app-primary)]' />
                  Shipment balance resets on the billing cycle so teams know when
                  allowances refresh.
                </li>
              </ul>
              <Link
            ...
            <div className='rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-8 shadow-sm md:p-10'>
              ...
              <p className='mt-3 text-sm leading-relaxed text-[var(--app-muted)]'>
                Accounts includes AWB products, tax invoices, and vouchers so
                finance artifacts stay tied to the same tenant as shipment
                activity—ideal for teams that want courier company admin software
                with operational context, not a standalone accounting tool.
              </p>
              ...
              <p className='mt-2 text-sm leading-relaxed text-[var(--app-muted)]'>
                Plans, trials, and usage reminders keep leadership aligned. Where
                in-product checkout is not the path, teams coordinate renewals
                offline—see{' '}
                <Link
                  href='/pricing'
                ...
                for numbers straight from the Plans &amp; upgrade experience.
              </p>
            </div>
          </div>
        </MotionInView>
